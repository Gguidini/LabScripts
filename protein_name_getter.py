...
def main():
        ...
        df['has_scorpion'] = [scorpio.get(df.iloc[idx]['LinkId'], 0) for idx in df.index]
        wasp = getCountsTotal(counts[3:6])
        df['has_wasp'] = [wasp.get(df.iloc[idx]['LinkId'], 0) for idx in df.index]
        spider = counts[6]
        df['has_spider'] = [spider.get(df.iloc[idx]['LinkId'], 0) for idx in df.index]
